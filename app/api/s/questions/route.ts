...
export async function GET() {
  ...
  const { data } = await db().from('questions').select('*').eq('student_id', sess.id).order('created_at', { ascending: false }).limit(100);
...
export async function POST(req: NextRequest) {
  ...
  let ai = '';
  try { ai = await askClaude(QA_SYSTEM, [{ role: 'user', content: String(question) }], 800); }
  catch { ai = '(AI 즉답 실패 — 질문은 저장됐어요.)'; }

  const enr = await activeEnrollmentOf(sess.id);
  const { data, error } = await db().from('questions')
    .insert({ student_id: sess.id, enrollment_id: enr?.id || null, question: String(question), ai_answer: ai })
    .select().single();
