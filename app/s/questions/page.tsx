...
type Q = { id: string; question: string; ai_answer: string; teacher_answer: string; ask_teacher: boolean; created_at: string };
...
export default function Questions() {
  ...
  async function ask() {
    ...
    setBusy(true);
    const res = await fetch('/api/s/questions', {
      method: 'POST',
      ...
      <div className="qa">
        ...
        {items.map((item) => (
          <div key={item.id} className="item">
            ...
            {item.ai_answer && <div className="a">{item.ai_answer}</div>}
            {item.teacher_answer && <div className="t-answer"><span className="who">선생님 답변</span>{item.teacher_answer}</div>}
            <div className="meta">
