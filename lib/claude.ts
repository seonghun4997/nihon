...
const L = (lang: Lang) => lang === 'en'
  ? { name: '영어', field: 'en', reading: '한국어 발음 표기 — 단어 단위로 띄어쓰기 (예: 쿠다이 겟 언 아일 씻)', readingWord: '한국어 발음 표기', level: '초중급 영어' }
  : { name: '일본어', field: 'jp', reading: '히라가나 읽기 — 의미 단위(어절)마다 띄어쓰기 (예: ここから あるいて なんぷん くらいですか)', readingWord: '히라가나', level: '초중급 일본어' };
...
- 모든 표현은 반드시 [${l.name} 원문 + 발음 + 한국어 뜻] 3요소를 갖춥니다. reading은 한국어 띄어쓰기처럼 의미 단위마다 공백을 넣습니다.
  ...
  "topics": [
    {
      ...
      "expressions": [ { "jp": "예상 표현 (${l.name})", "reading": "${l.reading}", "ko": "뜻", "gram": "문법 한 줄 (문형·활용)" } ],
...
- 모든 표현에 gram(문법 한 줄)을 답니다.
- reading은 의미 단위(어절)마다 띄어 씁니다 — 한국어 띄어쓰기처럼. 최근 수업과 자연스럽게 이어지되 겹치지 않게. 학습자의 실제 삶(일·일상·관심사)이 전사에 드러나면 반영.`;
};
...
규칙: expressions는 5개. 모든 표현에 gram을 답니다. reading은 의미 단위로 띄어 씁니다. 실제 대사를 인용하지 말고, 그 작품의 장면 유형(식당·이동·잡담 등)에서 통용되는 범용 표현으로. 초중급이 바로 써먹을 수 있는 것만.`;
};
...
규칙: 입력에 실제로 있는 것만, 최대 10개. 모든 항목에 gram을 답니다. reading은 의미 단위로 띄어 씁니다. 문장이 아니라 단어면 짧은 예문으로 확장.`;
};
